...
<<<<<<< HEAD
    ...
    if not args.apply:
        ...
        total_savings = 0.0
        for vol in volumes:
            savings = vol["size"] * PRICE_DELTA
            total_savings += savings
            print(f"  {vol['id']:<40} {vol['size']:>5}GB  attached={vol['attached']:<20} ${savings:.2f}/mo savings")
        print("-" * 78)
    ...
    for vol in volumes:
        ...
        print(f"  → modify_volume issued for {vol['id']} (gp3, 3000 IOPS, 125 MiB/s)")
