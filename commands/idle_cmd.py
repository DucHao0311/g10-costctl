...
<<<<<<< HEAD
    ...
    # Get all running instances, skip keep=true
        ...
        for reservation in page["Reservations"]:
            for inst in reservation["Instances"]:
                ...
                if tags.get("keep", "").lower() == "true":
                    continue
                instances.append((inst["InstanceId"], inst.get("InstanceType", "unknown")))
    ...
    for iid, itype in instances:
        ...
        if avg is None:
            ...
            if avg < threshold:
                idle_ids.append(iid)
        print(f"  {iid:<40} {itype:<12} cpu_{hours}h={cpu_str}{marker}")
